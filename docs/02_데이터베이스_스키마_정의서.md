...
| file_path     | VARCHAR(500) | NOT NULL                              | 파일 디렉토리 경로                  |
...
**최적화 포인트**: 영향평가에 집중, 파일 추적에 필요한 정보만 포함
...
| rel_type        | VARCHAR(50) | NOT NULL                              | 관계 타입 (CALL_METHOD, CALL_QUERY, USE_TABLE, JOIN_EXPLICIT, JOIN_IMPLICIT 등)                 |
| created_at      | DATETIME    | DEFAULT (datetime('now', '+9 hours')) | 생성일시 (KST)                                                                                   |
| updated_at      | DATETIME    | DEFAULT (datetime('now', '+9 hours')) | 수정일시 (KST)                                                                                   |
| del_yn          | CHAR(1)     | DEFAULT 'N'                           | 삭제 여부 (Y/N)                                                                                   |
...
-- 1. PK 컬럼을 src_id에 할당 (users.user_id가 PK)
-- 2. FK 컬럼을 dst_id에 할당 (orders.user_id가 FK)
...
### 인코딩 처리 방식
- **CSV 로딩**: 자동 인코딩 감지 (chardet 라이브러리 사용)
  - 감지 순서: 감지된 인코딩 → cp949 → euc-kr → utf-8 → latin-1
  - 실패 시 다음 인코딩으로 자동 전환
- **CSV 저장**: CP949 인코딩 고정
  - 이유: Microsoft Excel이 CP949 인코딩을 기본으로 읽음
  - 한글 호환성 최적화

### CSV 데이터 타입 처리
- **NUMBER(precision,scale)** 형태는 따옴표로 감싸기
  - 예: `"NUMBER(10,2)"` (쉼표 구분자 충돌 방지)
- **특수문자 포함 필드**: 자동 따옴표 처리 (csv.QUOTE_MINIMAL)

### 스키마 파일 구조
```
projects/SampleSrc/db_schema/
├── ALL_TABLES.csv        # 테이블 정보 (CP949)
└── ALL_TAB_COLUMNS.csv   # 컬럼 정보 (CP949)
```
...
- **오류 추적**: has_error, error_message로 오류 상태 관리
- **시간대**: 한국 시간대(KST) 적용 (UTC+9)
