...
sequenceDiagram
    ...
    X->>X: XML 파싱 + 즉시 관계 도출
    X->>D: 쿼리-테이블 관계 즉시 저장
    Note over X,D: 파싱과 동시에 관계 생성
    
    J->>J: Java 파싱 + 즉시 관계 도출
    J->>D: 메서드 호출 관계 즉시 저장
    Note over J,D: 파싱과 동시에 관계 생성
    
    F->>F: Frontend 파싱 + 즉시 관계 도출
    F->>D: API 호출 관계 즉시 저장
    Note over F,D: 파싱과 동시에 관계 생성
    
    R->>D: 중복 관계 제거 및 통합
    Note over R,D: 최종 관계 네트워크 완성
```
